...
const jetons: Record<string, { email: string; motsUtilises: Record<string, number> }> = {};
...
serveur.post("/api/token", (req: any, res: any) => {
  const email: string | undefined = req.body?.email;

  if (!email) return res.status(400).send({ erreur: "Email manquant" });
...
serveur.post("/api/justify", (req: any, res: any) => {
  const rawAuth = req.headers["authorization"];
  const auth = Array.isArray(rawAuth) ? rawAuth[0] : rawAuth;

  if (!auth) return res.status(401).send({ erreur: "Aucun jeton fourni" });

  const jeton = auth.replace("Bearer ", "").trim();
  if (!jetons[jeton]) return res.status(401).send({ erreur: "Jeton invalide" });

  const texte: string = typeof req.body === "string" ? req.body : "";
  const mots: string[] = texte.split(/\s+/).filter((m) => m.length > 0);

  const nbMots = mots.length;
  const jour = getToday();

  if (!jetons[jeton].motsUtilises[jour]) jetons[jeton].motsUtilises[jour] = 0;
  ...
  if (ligne) resultat += ligne;

  res.type("text/plain").send(resultat);
});

serveur.get("/", (_req: any, res: any) => {
